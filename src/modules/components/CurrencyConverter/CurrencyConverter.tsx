import { Box, FormControl, IconButton, Input, InputLabel, MenuItem, Select, TextField } from "@mui/material";
import CompareArrowsIcon from '@mui/icons-material/CompareArrows';
import { useState } from "react";
import { useAppSelector } from "../../../shared/hooks/redux-hooks";
import { arrCodeISO, codeUAH, currencies } from "../../../shared/constants/codeSelectedCurrencies";
...
const CurrencyConverter = (): JSX.Element => {
        ...
        if (currencyGet !== currencyChange) {
            const rate = currency.data.find(item => {
                return currencyGet === codeUAH ?
                    item.currencyCodeB === currencyGet && item.currencyCodeA === currencyChange :
                    item.currencyCodeA === currencyGet && item.currencyCodeB === currencyChange
            })
            if (rate) {
                const rez = currencyGet === codeUAH ? value / rate.rateSell : value * rate.rateSell;
    ...
    const handlerOnChangeCurrencyChange = (event: React.ChangeEvent<HTMLInputElement>) => {
        const value = Number(event.target.value);
        setValueChange(value);
        ...
        <Box sx={{
            ...
            <FormControl sx={{ m: 1 }} variant="standard">
                ...
                <Input
                    ...
                    value={valueChange} onChange={handlerOnChangeCurrencyChange}
                />
            </FormControl>
            <FormControl sx={{ m: 1, minWidth: 120 }} size="small">
                ...
                >
                    {arrCodeISO.map((item, index) => {
                        return (<MenuItem key={index} value={item}>{currencies[item.toString()]}</MenuItem>)
                    })}
            ...
            <FormControl sx={{ m: 1 }} variant="standard">
                ...
                <Input
                    type="number"
                    id="inputChangeGet"
                    value={valueGet} onChange={handlerOnChangeGet}
                />
            </FormControl>
